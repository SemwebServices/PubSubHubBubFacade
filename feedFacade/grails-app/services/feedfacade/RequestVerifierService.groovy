    ...
    try {
      ...
      while ( cont ) {
        ...
        PendingRequest.withNewTransaction {
          def q = PendingRequest.executeQuery('select pr.id, pr.mode, pr.callback, pr.topic, pr.guid, pr.leaseSeconds, pr.trimNs, pr.targetMimetype,pr.deliveryMode from PendingRequest as pr where pr.status = :pending order by pr.requestTimestamp asc',[pending:'pending'],[lock:true])
          if ( q.size() > 0 ) {
            def row = q.get(0)
            request_info = [:]
            ...
            request_info.deliveryMode = row[8]
